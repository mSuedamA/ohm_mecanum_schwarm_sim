...
from ohm_robot_swarm_sim.srv import Spawn, Kill, SpawnRequest, SpawnResponse, KillRequest, KillResponse, Dispersion, DispersionResponse, Assemble, AssembleResponse
...
class Ohm_Robot_Swarm_Simulator:
    ...
    def adjust(self, robot, destination):
        ...
        randdes = [0,0]
        while(True):
            randdes = [random.randint(-4,-2),random.randint(2,4)]
            for j in i_points:
                j[0] += randdes[0]
                j[1] += randdes[1]
